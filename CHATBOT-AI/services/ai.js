...
function getModelName(provider) {
  ...
  if (provider === 'gemini') {
    // Use user-defined model if it matches gemini prefix, fallback to gemini-2.5-flash
    return envModel.startsWith('gemini') ? process.env.AI_MODEL : 'gemini-2.5-flash';
  }
  if (provider === 'openai') {
    // Use user-defined model if it matches gpt/o1/o3, fallback to gpt-4o
    return (envModel.startsWith('gpt') || envModel.startsWith('o1') || envModel.startsWith('o3')) ? process.env.AI_MODEL : 'gpt-4o';
  }
  if (provider === 'claude') {
    // Use user-defined model if it matches claude, fallback to claude-3-5-sonnet-latest
